...
try:
    # 1. Fetch Fuel Types dynamically to map IDs
    ...
    # 4. Clean and Merge the Math
    prices_df = prices_df[prices_df['Price'] != 9999.0] # Remove out of stock fuel
    prices_df['Price'] = prices_df['Price'] / 1000.0 # Convert tenths of a cent to normal dollars
    prices_df['FuelType'] = prices_df['FuelId'].map(fuel_mapping)
    prices_df = prices_df.dropna(subset=['FuelType']) 
    
    # Pivot so each servo has a column for U91, U95, U98, Diesel
    pivot_prices = prices_df.pivot(index='SiteId', columns='FuelType', values='Price').reset_index()
    
    # Merge physical locations with the new prices
    final_df = pd.merge(sites_df, pivot_prices, on='SiteId', how='inner')
    
    # Fill missing fuels with 0.00 so the app doesn't crash if a servo only sells Diesel
...
with col1:
manual_address = st.text_input("Search Location", placeholder="e.g. Marion SA", help="Type a suburb or address and press Enter.", label_visibility="collapsed")
with col2:
loc = streamlit_geolocation()

st.markdown("### 2. What fuel do you need?")
fuel_choice = st.selectbox("Select Fuel Type", options=["U91", "U95", "U98", "Diesel"], help="Prices on the map will update to show this fuel type.", label_visibility="collapsed")
